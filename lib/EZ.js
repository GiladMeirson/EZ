...
function typeText(text='write your text here..', placeholderElement=document.body, speed=500) {
	placeholderElement.innerHTML='';
	const textContainer = document.createElement("span"); // Create a new div element
	placeholderElement.appendChild(textContainer); 
	let i =0;
	function type() {
		if (i < text.length) {
			textContainer.textContent += text.charAt(i);
			i++;
			setTimeout(type, speed); // Adjust the typing speed (e.g., 100 milliseconds in this example)
		}
	}

	type(); // Call the inner type function to start typing
}




const typingIt=(text='write here your text',tag='body')=>{
...
const pop =(title='YOUR TITLE HERE',content='YOUR CONTENT HERE',state='success')=>{
	Swal.fire(
		title,
		content,
		state,
...
const Cpop =(icon='error',title='YOUR TITLE HERE',content='YOUR CONTENT HERE',footer='<a href="#">Link Example</a>')=>{
	Swal.fire({
		icon: icon,
		title: title,
		text: content,
...
const Hpop=(icon='info',title='<strong>YOUR <u>TITLE</u> HERE</strong>',html='<a href="#">USE HERE IN HTML TAGS</a> ',confirmButtonText='OK',cancelButtonText='CANCEL')=>{
	Swal.fire({
		title: title,
		icon: icon,
		html:html,
		...
		focusConfirm: false,
		confirmButtonText:`<i class="fa fa-thumbs-up"></i>${confirmButtonText}`,
		confirmButtonAriaLabel: 'Thumbs up, great!',
		cancelButtonText:`<i class="fa fa-thumbs-down"></i>${cancelButtonText}`,
		cancelButtonAriaLabel: 'Thumbs down'
	  })
}

const Tpop=(title='YOUR TITLE HERE',timer=3000,html='I will close in <b></b> seconds.',CB=defultCallBack)=>{
	let timerInterval
	Swal.fire({
	title: title,
	html: html,
...
const Dpop=(icon='warning',title='Are you sure?',content='You wont be able to revert this!',CB=defultCallBack,confirmText='Yes',cancelColor='#d33',confirmColor='#308506',afterTitle='Deleted!',afterContent='Your file has been deleted.')=>{
	Swal.fire({
		title: title,
		text: content,
		icon: icon,
...
const copyThis=(text='put here the text you want to copy',sucssesCB=defultCallBack,ErrorCB=defultCallBack)=>{
...
// wire in init
const handleExcelToJson=(inputFile,callBack=defultCallBack)=>{
	if (typeof(inputFile)!='object') {
		return false;
	}
	inputFile.addEventListener('change',(e)=>{
		const file = e.target.files[0];
		readExcelInternal(file,callBack);
	})
}

//or manual wiring in onchange event !
const handleExceltoJsonOnChange=(callBack=defultCallBack)=>{
	var file = this.event.target.files[0];
	readExcelInternal(file,callBack);

}

const readExcelInternal = async (file,callback) => {
